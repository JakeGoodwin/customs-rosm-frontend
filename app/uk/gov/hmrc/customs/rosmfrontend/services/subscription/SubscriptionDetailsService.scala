...
class SubscriptionDetailsService @Inject()(
  ...
  def cacheNameIdAndCustomsId(name: String, id: String)(implicit hc: HeaderCarrier, request: Request[_]): Future[Unit] =
    saveSubscriptionDetails(
      sd => sd.copy(nameIdOrganisationDetails = Some(NameIdOrganisationMatchModel(name, id)), customsId = Some(Utr(id)))
    )

  def cachedNameIdDetails(implicit hc: HeaderCarrier, request: Request[_]): Future[Option[NameIdOrganisationMatchModel]] =
  ...
  def cacheDateOfBirth(date: LocalDate)(implicit hc: HeaderCarrier, request: Request[_]): Future[Unit] =
  ...
  def cacheEoriNumber(eoriNumber: String)(implicit hc: HeaderCarrier, request: Request[_]): Future[Unit] =
  ...
  def cachePersonalDataDisclosureConsent(consent: Boolean)(implicit hc: HeaderCarrier, request: Request[_]): Future[Unit] =
  ...
  def cacheIdDetails(idMatchModel: IdMatchModel)(implicit hc: HeaderCarrier, request: Request[_]): Future[Unit] =
    saveSubscriptionDetails(sd => sd.copy(idDetails = Some(idMatchModel)))

  def cacheCustomsId(subscriptionCustomsId: CustomsId)(implicit hc: HeaderCarrier, request: Request[_]): Future[Unit] =
    saveSubscriptionDetails(sd => sd.copy(customsId = Some(subscriptionCustomsId)))

  def clearCachedCustomsId(implicit hc: HeaderCarrier, request: Request[_]): Future[Unit] =
  ...
  def cacheExistingEoriNumber(eori: String)(implicit hc: HeaderCarrier, request: Request[_]): Future[Unit] =
    saveSubscriptionDetails(sd => sd.copy(existingEoriNumber = Some(eori)))

  def cachedExistingEoriNumber(implicit hc: HeaderCarrier, request: Request[_]): Future[Option[String]] =
